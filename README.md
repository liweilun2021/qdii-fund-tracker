...
- 持仓占比列动态插入，支持排序
- 展开行查看基金前十大持仓明细（含持仓报告期）
- 净值日期、持仓截止日期等数据时效性标注
- 多条件组合筛选，支持一键清除
- 悬浮刷新按钮，刷新完成/失败通知提示
- 本地文件缓存，重启秒加载；刷新失败时自动继承旧数据
...
首次启动会自动爬取数据（约 2-3 分钟全部完成），完成前页面显示缓存数据。
...
- 页面右下角悬浮按钮可手动触发全量刷新，等待所有数据（净值、申购状态、持仓）抓取完成后才更新
- 部分请求失败时自动从旧缓存继承，确保数据稳定不丢失
